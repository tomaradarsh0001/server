    ...
    <tr>
        ...
        <td>
            <div class="cursor-pointer text-primary" data-bs-toggle="modal"
                data-bs-target="#exampleScrollableModal{{ $propertyDetail->id }}">
                {{ $propertyDetail->unique_propert_id }}</div>
        ...
        <td>
            @foreach ($propertyDetail->splitedPropertyDetail as $key => $chldProperty)
                <p>
                    <a
                        href="{{ url('property-details/child/' . $chldProperty->id) }}">{{ $chldProperty->child_prop_id }}</a>
            ...
            <div class="d-flex gap-3">
                ...
                @haspermission('delete.property.details')
                    <button type=" button" data-bs-toggle="modal"
                        data-bs-target="#deleteProperty_{{ $propertyDetail->id }}"
                        class="btn btn-danger px-5">Delete</button>
                    <div class="modal fade" id="deleteProperty_{{ $propertyDetail->id }}" tabindex="-1"
                        aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                ...
                                <div class="modal-body">Do you really want to delete this property?</div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                    <form method="post"
                                        action="{{ route('property.destroy', ['id' => $propertyDetail->id]) }}">
                                        @csrf
                                        @method('delete')
