        ...
        <select id="{{ $prefix }}logistic_items_id{{ $prefix !== '' ? ']' : '' }}"
            ...
            @foreach($purchaseItem as $data)
                @if($data->status != 'inactive')
                    <option value="{{ $data->id }}" {{ isset($item) && $item->logistic_items_id == $data->id ? 'selected' : '' }}>
                        {{ $data->name }}
                    </option>
                @endif
            @endforeach
    ...
    <div class="form-group">
        ...
        <select id="{{ $prefix }}category_id{{ $prefix !== '' ? ']' : '' }}"
            ...
            @foreach($purchaseCategory as $data)
                @if($data->status != 'inactive')
                    <option value="{{ $data->id }}" {{ isset($item) && $item->category_id == $data->id ? 'selected' : '' }}>
                        {{ $data->name }}
                    </option>
                @endif
            @endforeach
...
<div class="col-md-2">
    <div class="form-group">
        <label class="form-label" for="{{ $prefix }}purchased_unit{{ $prefix !== '' ? ']' : '' }}">Purchased Unit:</span></label>
...
<div class="col-md-2">
    <div class="form-group">
        <label class="form-label" for="{{ $prefix }}reduced_unit{{ $prefix !== '' ? ']' : '' }}">Reduced Unit:</label>
