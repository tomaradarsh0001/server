 ...
 */
    ...
    $('input[name="new_allotment_radio"]').change(function() {
      ...
      if (selectedVal != '')
      var responseUrl = "{{ route('getDemandHeads', ['newAllotment' => '__VAL__']) }}";
            responseUrl = responseUrl.replace('__VAL__', selectedVal);    
        $.ajax({
          type: "get",
              url:responseUrl,
            success: function(response) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if(!(landRate && landArea)){
            showError('Can not calculate commercail land value. Please try again');
            return false;
          }
    ...
    function calculateLUCCharges(inputElements) {
      ...
      if (mixedUse) {
        ...
        if (tbuac > landArea) {
          $('#luc_TBUA_error').text(`Total build up area can not be more than land size ${landArea} Sq.m.`);
          inputError = true;
        }
        if (tbuac != "" && tbuac > 0) {
        ...
        if (buac && buac > 0) {
          if (buac > tbuac) {
            $('#luc_BUAC_error').text('Commercial area can not be more than total built up area');
            inputError = true;
          }
