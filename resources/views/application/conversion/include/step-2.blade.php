<div class="mt-3">
    <div class="container-fluid">
        <div class="row g-2">
            <div class="col-lg-12">
                @php
                    /* print_r($userDetails->applicantUserDetails->isIndian);
                    print_r($userDetails->applicantUserDetails);
                    echo '</pre>'; */
                ...
                @foreach ($stepTwoDocs as $document)
                    @if (
                        !empty($userDetails->applicantUserDetails->isIndian) &&
                            ...
                            ($document['id'] == 'convDocApplicantAadhaar' || $document['id'] == 'convDocApplicantPan'))
                        @php
                            $isIndian = true;
                        @endphp
                    @endif
                    ...
                    @if ($isIndian)
                        @if ($document['id'] == 'convNriUploadDocument')
                            @continue
                        @endif
                    @else
                        @if ($document['id'] == 'convDocApplicantAadhaar' || $document['id'] == 'convDocApplicantPan')
                            @continue
                        @endif
                    @endif
                        ...
                        <div class="col-lg-11 selected-docs-field">
                                ...
                                <div id="{{ $document['id'] }}_repeater" class="position-relative doc-items">
                                    <div class="position-sticky text-end mt-2 {{ $document['multiple'] }}"
                                        ...
                                        <button type="button" class="btn btn-primary repeater-add-btn"
                                            ...
                                            title="Click to add one more document below"><i
                            ...
                            @php
                                ...
                                if (!empty($application->tempDocument)) {
                                    $uploadedDocuments = $application->tempDocument
                            ...
                            <div class="duplicate-field-tab">
                                ...
                                @forelse($uploadedDocuments as $ud)
                                    ...
                                    <div class="{{ $document['multiple'] == 1 ? 'items' : '' }}"
                                        data-group="{{ $document['id'] }}_conversion" data-type="document"
                                        data-document-type="{{ $document['id'] }}">
                                        <input type="hidden" data-name="indexNo"
                                            value="{{ $ud->index_no ?? $loop->iteration }}"><!-- if index-no is not preset (for old data). then assign loop count-->
                                        ...
                                        <div class="item-content mb-2">
                                            <div class="row">
                                                @if (!isset($document['isFirstInput']) || $document['isFirstInput'] === true)
                                                    <div
                                                        class="col-lg-4 {{ empty($document['inputs']) ? 'icon-feild' : '' }}">
                                                        <div class="row align-items-center">
                                                            <div class="col-lg-9">
                                                                <div class="form-group form-box">
                                                                    <!-- added class form-label by anil on 17-04-2025 -->
                                                                    <label for="{{ $document['id'] }}"
                                                                        class="quesLabel form-label">{{ $document['label'] }}<span
                                                                            class="text-danger">*</span></label>
                                                                    <input type="hidden"
                                                                        name="{{ $document['id'] . 'Id' }}"
                                                                        value="{{ $ud['id'] }}"
                                                                        data-name="id"><!--To be used in edit -->
                                                                    <input type="file" name="{{ $document['id'] }}"
                                                                        class="form-control" accept="application/pdf"
                                                                        id="{{ $document['id'] }}"
                                                                        @if (!$document['multiple']) onchange="handleFileUpload(this.files[0], '{{ $document['label'] }}','{{ $document['id'] }}', 'conversion', 'CONVERSION')" @endif
                                                                        data-name="{{ $document['id'] }}"
                                                                        data-should-validate = "{{ isset($ud) && $ud['file_path'] != '' }}">
                                                ...
                                                @if (isset($document['inputs']))
                                                    @foreach ($document['inputs'] as $input)
                                                        @php
                                                            $uploadedValue = $ud->tempDocumentKeys
                                                                ->where('key', $input['id'])
                                                                ->first();
                                                        @endphp
                                                        <div
                                                            class="col-lg-4 {{ $document['multiple'] && $loop->iteration === count($document['inputs']) ? 'icon-feild' : '' }}">
                                                            <div class="row align-items-center">
                                                                <div class="col-lg-12">
                                                                    <div class="form-group">
                                                                        <!-- added class form-label by anil on 17-04-2025 -->
                                                                        <label for="dateattestation"
                                                                            class="form-label">{{ $input['label'] }}<span
                                                                                class="text-danger">*</span></label>
                                                                        @if ($input['type'] == 'select')
                                                                            <select name="{{ $input['id'] }}"
                                                                                ...
                                                                                @foreach ($input['options'] as $option)
                                                                                    <option value="{{ $option }}"
                                                                                        {{ isset($uploadedValue) && $uploadedValue->value == $option ? 'selected' : '' }}>
                                                                                        {{ $option }}</option>
                                                                                @endforeach
                                                                            </select>
                                                                    ...
                                                                    <div class="col-lg-3">
                                                                        <div class="repeater-remove-btn"
                                                                            ...
                                                                            <button type="button"
                                                                                ...
                                                                                data-placement="bottom"
                                                                                title="Click here to delete this document."
                                                                                onclick="removeRepeater($(this).parents('.items'),'{{ $ud->index_no ?? $loop->iteration }}')">
                                                                                <i
                                                                                    class="fadeIn animated bx bx-x-circle"></i>
                                                                            </button>
                                                                        </div>
                                                        ...
                                                        @if (isset($document['isFirstInput']) &&
                                                                $document['isFirstInput'] === false &&
                                                                $document['displayAtIndex'] == $loop->iteration)
                                                            <div class="col-lg-4">
                                                                <div class="form-group form-box">
                                                                    <!-- added class form-label by anil on 17-04-2025 -->
                                                                    <label for="{{ $document['id'] }}"
                                                                    ...
                                                                    <input type="file"
                                                                        ...
                                                                        class="form-control" accept="application/pdf"
                                                                        id="{{ $document['id'] }}"
                                                                        onchange="handleFileUpload(this.files[0], '{{ $document['label'] }}','{{ $document['id'] }}', 'conversion', 'CONVERSION')"
                                                                        data-name="{{ $document['id'] }}"
                                                                        data-should-validate = "{{ isset($ud) && $ud['file_path'] != '' }}">
                                                                    <a href="{{ asset('storage/' . $ud['file_path'] ?? '') }}"
                                                                        target="_blank" class="fs-6">View Saved
