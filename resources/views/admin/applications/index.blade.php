        ...
        <div class="card-body">
            <div class="d-flex justify-content-end">
                <ul class="d-flex gap-3 flex-wrap">
                    <!-- added class flex-wrap for fixed the form responsive issue by anil on 27-08-2025 -->
                    ...
                    <tr>
                        ...
                        <th>
                            <select class="form-control form-select form-select-sm" name="filterByApplication"
                                ...
                                @foreach ($appTypeItems as $item)
                                    <option class="text-capitalize" value="{{ $item->id }}"
                                        @if ($getApplicationTypeId == $item->id) @selected(true) @endif>
                                        {{ $item->item_name }}
                                    </option>
                                @endforeach
                            </select>
                        </th>
                        <th>Application Current With</th>
                        <th>Activity</th>
                        <th>
                            <select class="form-control form-select form-select-sm" name="status" id="status"
                                style="font-weight: bold; width:109px;">
                                <option value="">Status</option>
                                @foreach ($items as $item)
                                    @if ($item->item_code != 'APP_DES')
                                        <option class="text-capitalize" value="{{ $item->id }}"
...
@section('footerScript')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            var table = $('#example').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'file_current_location',
                        name: 'file_current_location'
                    },
                    {
                        data: 'activity',
                        name: 'activity',
            ...
            $('#filterByApplication').change(function() {
                table.ajax.reload();
            });
        ...
        function handleViewApplication(applicationNo, type, applicationId) {
            ...
            $.ajax({
                url: "{{ route('applications.applicationCanView') }}",
                type: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    applicationNo: applicationNo
