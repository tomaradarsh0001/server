...
use Illuminate\Support\Facades\Hash; //added by swati mishra for resend communication on 18082025
...
class OfficialController extends Controller
{
    ...
    public function getRegisteredUsers(Request $request)
    {
        ...
        foreach ($getRegistrationDetails as $key => $getRegistrationDetail) {
            $autoForward = '';
            if ($getRegistrationDetail->forward_through == 'cron' && $getRegistrationDetail->action_taken_by == 'deputy-lndo') {
            ...
            if (!Is_null($assignedToUSer)) {
                ...
                if ((in_array($getRegistrationDetail->applicant_number, $earliest))) // code modfied by nitin on 03-09-2025
                {
                    if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                        ...
                        if ($getRegistrationDetail->action_taken_by == 'section-officer') {
                            $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertGreen"></div></div><div style="color:#116d6e;">' . $autoForward . '</div>';
                        } else {
                            $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '</div><div style="color:#116d6e;">' . $autoForward . '</div>';
                        }
                    }
                } else if (
                    in_array($getRegistrationDetail->applicant_number, $assignedToUSer)
                    //&&  (in_array($getRegistrationDetail->applicant_number, $earliest)) // code modfied by nitin on 03-09-2025
                ) {
                    if ($getRegistrationDetail->forward_through == 'cron' && $roleName == 'deputy-lndo') {
                        $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertRed"></div></div><div style="color:#116d6e;">' . $autoForward . '</div>';
                    } else {
                        $nestedData['applicant_number'] =  '<div class="d-flex gap-2 align-items-center">' . $getRegistrationDetail->applicant_number . '<div class="alertDot"></div></div><div style="color:#116d6e;">' . $autoForward . '</div>';
                    }
                }
            }
        ...
        try {
            ...
            if ($iseditedOrApprovedEver) {
                ...
                if ($applicationStatus) {
                    ...
                    if ($request->serviceType == 'RS_NEW_REG') {
    ...
    public function propertyTransferSection(Request $request)
    {
        ...
        if (empty($request->propertyMasterId) || empty($request->sectionId) || empty($request->remarks)) {
            return response()->json(['status' => 'failed', 'message' => 'Property ID, Section ID & Remarks are required']);
        }
        ...
        return DB::transaction(function () use ($request) {
            ...
            $propertyMaster->property_transfer_remark = $request->remarks;
