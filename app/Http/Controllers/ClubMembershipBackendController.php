...
class ClubMembershipBackendController extends Controller
{
    ...
    {
        $items = Item::where('group_id', 17012)->where('is_active', 1)->whereIn('item_code', ['CM_REJ', 'CM_APP'])->get();
    ...
    {
        $itemsIdArr = Item::where('group_id', 17012)
            ->where('is_active', 1)
            ->whereIn('item_code', ['CM_REJ', 'CM_APP'])
            ->pluck('id')
            ->toArray();
        ...
        if (!empty($request->status)) {
            ...
            $query->whereIn('club_membership_applications.status', ($itemsIdArr));
        }

        $columns = ['id', 'unique_id', 'name', 'category', 'club_type', 'designation', 'name_of_service', 'year_of_allotment', 'date_of_application', 'date_of_joining_central_deputation', 'pay_scale', 'other_relevant_information', 'status_name', 'created_by', 'updated_by', 'created_at'];
        ...
        if ($order === 'unique_id') {
            // Sort by numeric value after CMA prefix
            $query->orderByRaw("CAST(SUBSTRING(club_membership_applications.unique_id, 4) AS UNSIGNED) $dir");
        } elseif ($order === 'status') {
    ...
    public function indexReceived(Request $request)
    {
        ...
        $items = Item::where('group_id', 17012)->where('is_active', 1)->whereIn('item_code', ['CM_NEW', 'CM_PEN', 'CM_INP'])->get();
        ...
        return view('club_membership.index_received', compact('items', 'getStatusId'));
    }

    public function getClubMembershipReceivedList(Request $request)
    {
        $itemsIdArr = Item::where('group_id', 17012)
            ->where('is_active', 1)
            ->whereIn('item_code', ['CM_NEW', 'CM_PEN', 'CM_INP'])
            ->pluck('id')
            ->toArray();
        ...
        // Order
        if ($order === 'serial_number' || $order === 'action') {
            // Skip ordering on fake columns
        } elseif ($order === 'unique_id') {
            // Sort by numeric value after CMA prefix
            $query->orderByRaw("CAST(SUBSTRING(club_membership_applications.unique_id, 4) AS UNSIGNED) $dir");
        } elseif ($order === 'status') {
            $query->orderBy('items.item_name', $dir);
        ...
        foreach ($getClubMembershipData as $clubMembership) {
            ...
            $nestedData['id'] = $clubMembership->id;
            $nestedData['unique_id'] = $clubMembership->unique_id;
            $nestedData['name'] = $clubMembership->name;
            $nestedData['club_type'] = $clubMembership->club_type;
            if (!empty($clubMembership->designation_equivalent_to) && $clubMembership->designation_equivalent_to === 'OTHER') {
    ...
    public function getClubMembershipDetails($clubMembershipId)
    {
        ...
        try {
            ...
            //Check If document is available for club membership
            $checkUploadedDocumentAvailable = false;
            if (!empty($getClubMembershipDetails->club_type)) {
                if ($getClubMembershipDetails->club_type === 'IHC') {
                    if (!empty($getClubMembershipDetails->ihcDetails->ihcs_doc)) {
                        $checkUploadedDocumentAvailable = true;
                    }
                } else {
            ...
            if ($sectionId && in_array($sectionId, $sectionIdArr) && $user->hasPermissionTo('club.membership.action') && $checkUploadedDocumentAvailable) {
    ...
    public function store(Request $request)
    {
        $rules = [
            ...
            'document' => 'required|mimes:pdf|max:2048', // 5120 KB = 5 MB
        ];
        ...
        if ($checkExist) {
            return redirect('public-services/club-membership/received-applications')->with('failure', 'Club Membership application already exists for the same email and mobile.');
        }
        ...
        if (isset($document) && $document != '') {
            ...
            // $document = GeneralFunctions::uploadFile($document, '/clubmem', $docName);
            $maxId = ClubMembership::max('id');
            $maxId = $maxId + 1;
            $timestamp = now()->format('Ymd_His');
            $docName = "{$maxId}_{$request->club_type}_{$timestamp}.pdf";
            $document = GeneralFunctions::uploadFile($document, '/clubmem', $docName);
        }
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($club) {
            $club->membership_id = $request->membershipId;
    ...
    public function allotmentClubMembership(Request $request)
    {
        ...
        return response()->json([
            ...
            'redirect_url' => route('club.membership.received.index')
        ]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if (isset($document) && $document != '') {
            ...
            $timestamp = now()->format('Ymd_His');
            $docName = "{$request->clubMembershipId}_{$request->club_type}_{$timestamp}.pdf";
            $document = GeneralFunctions::uploadFile($document, '/clubmem', $docName);
        }
    ...
    public function sendBulkEmailMembership(Request $request)
    {
        try {
            $request->validate([
                'ids' => 'required|array',
                'ids.*' => 'exists:club_membership_applications,id',
            ]);

            $members = DB::table('club_membership_applications')
                ->whereIn('id', $request->ids)
                ->get();

            foreach ($members as $member) {
                // Notification: Setup data and action
                $notificationData = [
                    'unique_id' => $member->unique_id,
                    'club_type' => $member->club_type,
                ];

                $action = (!empty($request->type) && $request->type == 'pending')
                    ? 'CLUB_MEM_PEN'
        ...
        try {
            ...
            Log::error('Bulk Email Membership Error: ' . $e->getMessage());
            return response()->json(['status' => 'failure', 'message' => 'An error occurred while sending emails.' . $e->getMessage()]);
        }
    }
