...
class ItemsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $action_link = '<a href="' . url("/logistic/items") . '" target="_blank">' . $logistic->name . '</a>';
        UserActionLogHelper::UserActionLog('create', url("/logistic/items"), 'logisticItems', "New logistic item " . $action_link . " has been created by " . Auth::user()->name.".");

        return redirect('logistic/items')->with('success', 'Logistic item created successfully.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $action_link = '<a href="' . url("/logistic/items") . '" target="_blank">' . $request->name . '</a>';
        UserActionLogHelper::UserActionLog('update', url("/logistic/items"), 'logisticItems', "Logistic item " . $action_link . " has been updated by " . Auth::user()->name.".");

        return redirect('logistic/items')->with('success', 'Data updated successfully');
    }
    ...
    public function updatelabel(Request $request, LogisticItem $item)
    {
        if ($request->ajax()) {
            ...
            $action_link = '<a href="' . url("/logistic/items") . '" target="_blank">' . $request->value . '</a>';
            UserActionLogHelper::UserActionLog('update', url("/logistic/items"), 'logisticItems', "Logistic item label " . $action_link . " has been updated by " . Auth::user()->name.".");
    ...
    public function destroy($id)
    {
        ...
        if ($item->delete()) {
            ...
            $action_link = '<a href="' . url("/logistic/items") . '" target="_blank">' . $item->name . '</a>';
            UserActionLogHelper::UserActionLog('delete', url("/logistic/items"), 'logisticItems', "Logistic item " . $action_link . " has been deleted by " . Auth::user()->name.".");
    ...
    public function updateStatus(Request $request, $itemId)
    {
        ...
        if ($pendingRequests) {
            return redirect()->back()->with('failure', 'Request related to this item is pending so it cannot be deactivated');
        }
        ...
        if ($purchasedItems) {
            return redirect()->back()->with('failure', 'Item cannot be deactivated as it has already been purchased.');
        }
