...
class LndoResidentialLandRatesImport implements ToModel, WithHeadingRow
{
    public function model(array $row): LndoResidentialLandRate|null
    {
        ...
        $existingRecord = LndoResidentialLandRate::where('colony_id', $oldColony->id)
        ...
        if ($existingRecord) {
            ...
            if (is_null($existingRecord->land_rate) && !is_null($row['residential_land_rate'])) {
                $existingRecord->land_rate = $row['residential_land_rate'];
                $existingRecord->save();
            }
            return null;
        }
        ...
        return new LndoResidentialLandRate([
            'colony_id' => $oldColony->id,
            'date_from' => $dateFrom,
            'date_to' => $dateTo,
            'land_rate' => $row['residential_land_rate'],
