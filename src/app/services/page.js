...
const getDynamicUrl = (title) => {
    const normalizedTitle = title?.toLowerCase().trim();
    const isNOC =
        normalizedTitle === "noc for registration" ||
        normalizedTitle === "पंजीकरण के लिए एनओसी" ||
        normalizedTitle === "conversion" ||
        normalizedTitle === "संपरिवर्तन" ||
        normalizedTitle === "Mutation" ||
        normalizedTitle === "दाखिल खारिज" ||
        normalizedTitle === "Deed of Apartment";
    normalizedTitle === "अपार्टमेंट का विलेख";

    return isNOC
        ? "https://ldo.mohua.gov.in/edharti"
        : "https://ldo.gov.in/edharti/";
};
            ...
            <div className="px-4 md:px-4 py-10 lg:px-8 xl:py-20 xl:px-10 2xl:px-24 2xl:py-12 lg:mt-[50px]">
                <div className="services-row grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-11 2xl:grid-cols-4 2xl:gap-7 xl::grid-cols-4 xl:gap-6 lg:grid-cols-4 lg:gap-4 md:grid-cols-3 md:gap-5 md:mt-12">
                    {servicesData.map((item, index) => {
                        const dynamicUrl = getDynamicUrl(item.servicesTitle);//added for dynamic apply url by Swati Mishra on 06052025
                        ...
                        return (
                            <div
                                ...
                                data-aos-duration="1000"
                            >
                                <Link href={dynamicUrl} target="_blank">
                                    <div className="w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 m-auto">
                                ...
                                <div className="services-link flex flex-col items-center mt-3 space-y-2">
                                    {/* Read More Button */}
                                    <button
                                        onClick={() => {
                                            openModal(index);
                                    ...
                                    >
                                        {Translate.readMore[lang]} <EyeIcon className="ml-1 w-3 sm:w-4" />
                                    </button>

                                    {/* Apply Now Button added by Swati Mishra on 02052025*/}
                                    <button onClick={() => window.open(dynamicUrl, "_blank")} className="text-sm sm:text-base inline-flex items-center justify-center underline text-blue-600">
                                        {Translate.ApplyNow[lang]} <ArrowRightIcon className="ml-1 w-3 sm:w-4" />
                                    </button>
                                </div>
                            </div>
                        );
                    ...
                    <div className="flex items-center justify-start">
                        <div className="w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32">
                            <div className="services-icon circle-sm circle2 text-center p-3 w-24 h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 m-auto flex items-center justify-center rounded-full bg-white md:p-4">
                                <Image
                                    src={HOST_NAME.replace('/admin', '') + activeModel.servicesIcon}
                                    alt="Substitution"
                                    className="w-12 h-12 md:w-auto md:h-auto object-cover m-auto"
                                    width={200}
                                    height={200}
                ...
                <div className="more-info-link p-2 mt-4 ml-5">
                    ...
                    {activeModel.link ? (
                        <Link
                            ...
                            href={
                                activeModel.link
                                    ? HOST_NAME.replace('/admin', '') + activeModel.link
                                    : "#"
                            }
                    ...
                    {/* SwatiMishra changed label from for apply to apply now on 02052025 */}
                    <Link
                        className="apply-btn text-sm md:text-lg px-5 py-2"
                        href={getDynamicUrl(activeModel.servicesTitle)}
                        target="_blank"
                    >
                        {Translate.ApplyNow[lang]}
                    </Link>
