...
// Skeleton Loading Component by Adarsh on 03 febuary 2025
const SkeletonLoader = () => {
    return (
        <div className="animate-pulse">
            <div className="flex opacity-50 items-center gap-6">
                ...
                <div className="hidden lg:flex lg:gap-x-4 xl:gap-x-6">
                    {[...Array(8)].map((_, index) => (
                        <div key={index} style={{ height: '32px', width: '120px' }}  className="bg-gray-300 rounded"></div>
                    ))}
                </div>
            </div>
        </div>
...
const getFixedLink = (link) => {
        if (!link) return "#";

        // Fix malformed external links
        link = link.replace(/^http:\/(?!\/)/i, "http://");
        link = link.replace(/^https:\/(?!\/)/i, "https://");

        // External URLs → leave as is
        if (/^https?:\/\//i.test(link)) return link;

        // Force internal URLs to be root-relative
        return "/" + link.replace(/^\/+/, "");
    };
    ...
    return (
        <header className="bg-white">
            <nav
                className="mx-auto flex max-w-full items-center justify-between p-4 lg:px-8 head-wrap"
                aria-label="Global"
            >
                <div className="flex head-logo">
                    <Link href={"/"} className="-m-1.5 p-1.5">
                        <span className="sr-only">
                            Land and Development Office
                        </span>
                        <Image
                            className="md:w-[300px] w-[300px] 2xl:w-[320px]"
                            width={300}
                ...
                <div className="flex items-center gap-6 main-menu">
                    <div className="flex lg:hidden ">
                        <button
                            type="button"
                            className="inline-flex items-center justify-center rounded-md py-2.5 pl-2 text-gray-700"
                            ...
                            {headerMenus.map((menu, index) => {
                                ...
                                if (menu.submenus) {
                                                                        ...
                                                                        >
                                                                            <div className="flex-auto">
                                                                                  <Link
                                                                                    ...
                                                                                    //  add close to closeing header menu after click by anil on 28-05-2025
                                                                                    onClick={() => {
                                                                                        close();
                                                                                        setOpenMenuIndex(null);
                                                                                    }}
                                    ...
                                    return (
                                        <Link
                                            key={index}
                                            href={getFixedLink(menu.link)}
                                            className="text-sm md:text-base lg:text-lg font-semibold leading-6 text-gray-900 outline-0"
                                            target={
                                        ...
                                        if (menu.submenus) {
                                            return (
                                                ...
                                                >
                                                    {({ open }) => (
                                                        <>
                                                            <Disclosure.Button className="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base font-semibold leading-6 hover:bg-gray-400/10">
                                                                {menuDisplay}
                                                                <ChevronDownIcon
                                            ...
                                            return (
                                                <Link
                                                    ...
                                                    className="flex w-full py-2 pr-3.5 text-base font-semibold leading-6 text-gray-900"
                                                    target={
                                                        menu.new_tab == "1"
                                                            ? "_blank"
                                                            : ""
