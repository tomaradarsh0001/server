...
function validateDoaOtherDoc() {
  var doaOtherDoc = document.querySelector(".otherDocuemntByApplicant");
  var doaOtherDocError = document.querySelector(".otherDocuemntByApplicantError");

  if (!doaOtherDoc || !doaOtherDocError) {
    console.warn("Input or error div with required class not found.");
    return false;
  }

  // If no file is selected, show error
  if (doaOtherDoc.files.length === 0) {
    doaOtherDocError.textContent = "Please select a document.";
    return false;
  }

  var file = doaOtherDoc.files[0];

  // Validate file size (max 5 MB)
  if (file.size > 5 * 1024 * 1024) {
    doaOtherDocError.textContent = "File size must be less than 5 MB.";
    return false;
  }

  // Validate file type (PDF only)
  if (!file.name.toLowerCase().endsWith(".pdf")) {
    doaOtherDocError.textContent = "Only PDF files are allowed.";
    return false;
  }

  // Clear error if all checks pass
  doaOtherDocError.textContent = "";
  return true;
}
