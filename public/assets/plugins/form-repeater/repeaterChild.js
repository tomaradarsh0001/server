...
validateButton.addEventListener("click", function () {
  ...
  // Validation for land transfer details for mis edit form only - Lalit (29/05/2025)
  const parentContainersNew = document.querySelectorAll(
    ".parent-container-new"
  );
  parentContainersNew.forEach((parentContainer) => {
    const isValid = multipleRepeater.validateInputFields(parentContainer);
    if (!isValid) {
      allValid = false;
    }
  ...
  if (allValid) {
    ...
    stepper3.next();
    const transferredCheckboxYes =
      document.getElementById("transferredFormYes");
    transferredCheckboxYes.addEventListener("change", function () {
      if (this.checked) {
        var addTransferBtnError = document.getElementById(
          "addTransferBtnError"
        );
        ...
        addTransferBtn.addEventListener("click", function () {
          ...
          addLesseeBtn.addEventListener("click", function () {
            var addLesseeBtnError =
              document.getElementById("addLesseeBtnError");
