jQuery.fn.extend({
  createRepeater: function (options = {}) {
    ...
    var addItem = function (items, key, fresh = false, indexValue = null) {
      ...
      var uniqfile = item.find("input[type='file']");

      input.each(function (index, el) {
        var attrName = $(el).data("name");
        var skipName = $(el).data("skip-name");
        // var inputFile = $(el).attr("type") == "file";
        // if(inputFile){
        //   $(el).attr("data-should-validate", 0);
        // }
        if (skipName != true) {
        ...
        if (fresh == true) {
          ...
          if ($(el).is("select")) {
              ...
              // Optionally set the first option to selected again (if you want to default it)
      ...
      uniqfile.each(function (index, el) {
        if (fresh == true) {
          $(el).attr("data-should-validate", '')
        }
      });
      ...
      // update condition by anil for edit case already have coapplicant disabled remove button and endable remove button when adding new coapplicant with using custom attirbute, on 16-04-2025
      if (key == 0 || (removeButton.attr("not-deletable")!== undefined && !fresh)) {
        // if (key == 0) {
        removeButton.attr("disabled", true); 
        // removeButtonWrap.hide();
      } else {
        removeButton.attr("disabled", false);
        // removeButtonWrap.show();
        /* Handling remove button conditions for edit mode for co applicant - Lalit Tiwari (25/03/2025) */
        /* if (removeButton.attr("editmode")) {
          if (fresh == true) {
            removeButton.attr("disabled", false);
          } else {
            removeButton.attr("disabled", true);
      ...
      */
      ...
      // added by anil for set max date only, without overwriting existing value on 09-04-2025
      var today = new Date().toISOString().split("T")[0];
      newItem.find('input[type="date"]').each(function () {
        $(this).attr("max", today);
    
        if (fresh === true && $(this).val() === "") {
          // keep empty, do not auto-fill
          $(this).val("");
        }
      });
    };
